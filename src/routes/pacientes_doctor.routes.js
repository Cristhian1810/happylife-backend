...
router.get(
  '/mis-pacientes/:pacienteId/historial',
  isDoctor,
  async (req, res) => {
    ...
    try {
      ...
      if (patientBelongsToDoctor.rowCount === 0) {
        return res
          .status(403)
          .json({ message: 'No tienes citas registradas con este paciente.' });
      }
...
router.post('/historial-medico', isDoctor, async (req, res) => {
  ...
  if (!cita_id || !diagnostico) {
    return res
      .status(400)
      .json({ message: 'La cita y el diagnóstico son obligatorios.' });
  }

  try {
    const citaCheck = await pool.query(
      'SELECT doctor_usuario_id, paciente_usuario_id, fecha_hora_inicio FROM citas WHERE id = $1',
      [cita_id]
    );
    ...
    const cita = citaCheck.rows[0];

    if (cita.doctor_usuario_id !== doctorId) {
      return res
        .status(403)
        .json({ message: 'No tiene permiso para registrar en esta cita.' });
    }
    ...
    if (fechaCita > ahora) {
      return res.status(400).json({
        message:
          'No se puede registrar historial de una cita futura. Espere a la hora de la cita.',
      });
    }
    ...
    await pool.query('UPDATE citas SET estado_cita_id = 3 WHERE id = $1', [
      cita_id,
    ]);

    res
      .status(201)
      .json({ message: 'Historial guardado con éxito.', entry: rows[0] });
  } catch (error) {
    console.error('Error al crear historial:', error);
    if (error.code === '23505') {
      return res.status(409).json({
        message: 'Ya existe un registro de historial para esta cita.',
      });
    }
    res.status(500).json({ message: 'Error interno del servidor.' });
  }
...
router.put('/citas/:citaId/estado', isDoctor, async (req, res) => {
  ...
  if (!estado_cita_id) {
    return res
      .status(400)
      .json({ message: 'El ID del nuevo estado es requerido.' });
  }
  ...
  try {
    ...
    if (rowCount === 0) {
      return res.status(404).json({
        message: 'Cita no encontrada o no tiene permiso para modificarla.',
      });
    }
    res
      .status(200)
      .json({ message: 'Estado de la cita actualizado correctamente.' });
  } catch (error) {
