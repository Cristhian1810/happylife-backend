...
router.post('/doctores', async (req, res) => {
  ...
  try {
    ...
    const userInsertQuery =
      'INSERT INTO usuarios (email, password_hash, dni, nombres, apellidos, telefono, rol_id) VALUES ($1, $2, $3, $4, $5, $6, 3) RETURNING id';
    ...
    const profileInsertQuery =
      'INSERT INTO perfiles_doctores (usuario_id, numero_colegiatura, biografia, titulo_profesional_id) VALUES ($1, $2, $3, $4)';
    ...
    res
      .status(201)
      .json({ message: 'Doctor registrado correctamente', usuarioId });
  } catch (error) {
...
router.put('/doctores/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      'DELETE FROM doctores_especialidades WHERE doctor_usuario_id = $1',
      [id]
    );
...
router.delete('/doctores/:id', async (req, res) => {
  ...
  try {
    const { rowCount } = await pool.query(
      'UPDATE usuarios SET esta_activo = false WHERE id = $1 AND rol_id = 3',
      [id]
    );
    if (rowCount === 0) {
      return res.status(404).json({ message: 'Doctor no encontrado.' });
